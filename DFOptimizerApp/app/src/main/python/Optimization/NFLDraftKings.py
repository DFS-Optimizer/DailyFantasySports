...
class Draftkings(Optimizer):
    ...
    def type_1(self, lineups):
        ...
        prob += (3 <= pulp.lpSum(self.positions['WR'][i] * players_lineup[i] for i in range(self.num_players)))
        prob += (pulp.lpSum(self.positions['WR'][i] * players_lineup[i] for i in range(self.num_players)) <= 4)
